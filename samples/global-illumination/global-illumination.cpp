...
public:
  ...
  :application_t("Global Illumination", 1200u, 800u, 3)
  {
    ...
    render::texture2DCreate(context, shadowMapSize_, shadowMapSize_, 1u, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT, render::texture_sampler_t(), &shadowMapRT0_);
    render::textureChangeLayoutNow(context, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, &shadowMapRT0_);
    render::texture2DCreate(context, shadowMapSize_, shadowMapSize_, 1u, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT, render::texture_sampler_t(), &shadowMapRT1_);
    render::textureChangeLayoutNow(context, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, &shadowMapRT1_);
    render::texture2DCreate(context, shadowMapSize_, shadowMapSize_, 1u, VK_FORMAT_R16G16B16A16_SFLOAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT, render::texture_sampler_t(), &shadowMapRT2_);
    render::textureChangeLayoutNow(context, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, &shadowMapRT2_);
    ...
    descriptor = render::getDescriptor(shadowMapRT0_);
    ...
    descriptor = render::getDescriptor(shadowMapRT1_);
    render::descriptorSetCreate(context, descriptorPool_, presentationDescriptorSetLayout_, &descriptor, &presentationDescriptorSet_[5]);
    descriptor = render::getDescriptor(shadowMapRT2_);
    render::descriptorSetCreate(context, descriptorPool_, presentationDescriptorSetLayout_, &descriptor, &presentationDescriptorSet_[6]);
  ...
  void addDirectionalLight(const maths::vec3& position, const maths::vec3& direction, const maths::vec3& color, float ambient)
  {
    if (directionalLight_ == nullptr)
    {
      ...
      directionalLight_->uniforms.direction = maths::vec4(lightDirection, 0.0f);
      directionalLight_->uniforms.color = vec4(color, ambient);
      ...
      directionalLight_->uniforms.worldToClipSpace = lightViewMatrix * orthographicProjectionMatrix(-1.0f, 1.0f, 1.0f, -1.0f, 0.01f, 2.0f);
      directionalLight_->uniforms.shadowMapSize = vec4((float)shadowMapSize_, (float)shadowMapSize_, 1.0f / (float)shadowMapSize_, 1.0f / (float)shadowMapSize_);

      //Generate sampling pattern
      float maxRadius = 25.0f;
      for (uint32_t i(0); i < directionalLight_->uniforms.sampleCount; ++i)
      {
        float e1 =  float((double)rand() / (RAND_MAX));
        float e2 =  float((double)rand() / (RAND_MAX));        
        directionalLight_->uniforms.samples[i] = vec4(maxRadius * e1 * sinf(2.0f *(float) PI * e2), maxRadius * e1 * cosf(2.0f * (float)PI * e2), e1*e1, 0.0f);
      }
      //Create uniform buffer and descriptor set
      render::gpuBufferCreate(context, render::gpu_buffer_t::UNIFORM_BUFFER,
...
private:
  ...
  void load(const char* url)
  {
    ...
    uint32_t meshCount = mesh::createFromFile(context, url, mesh::EXPORT_NORMALS_UVS, &allocator_, &mesh);
  ...
  void initializeShadowPass(render::context_t& context)
  {
    ...
    shadowAttachments[0].format = shadowMapRT0_.format;
    ...
    shadowAttachments[1].format = shadowMapRT1_.format;
    shadowAttachments[1].initialLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    shadowAttachments[1].finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    ...
    shadowAttachments[2].format = shadowMapRT2_.format;
    shadowAttachments[2].initialLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    shadowAttachments[2].finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    shadowAttachments[2].storeOp = VK_ATTACHMENT_STORE_OP_STORE;
    shadowAttachments[2].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    shadowAttachments[2].samples = VK_SAMPLE_COUNT_1_BIT;

    shadowAttachments[3].format = depthStencilBuffer_.format;
    shadowAttachments[3].initialLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    shadowAttachments[3].finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    shadowAttachments[3].storeOp = VK_ATTACHMENT_STORE_OP_STORE;
    shadowAttachments[3].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    shadowAttachments[3].samples = VK_SAMPLE_COUNT_1_BIT;

    render::render_pass_t::subpass_t shadowPass;
    shadowPass.colorAttachmentIndex.push_back(0);
    shadowPass.colorAttachmentIndex.push_back(1);
    shadowPass.colorAttachmentIndex.push_back(2);
    shadowPass.depthStencilAttachmentIndex = 3;
    ...
    //Create frame buffer
    VkImageView shadowFbAttachment[4] = { shadowMapRT0_.imageView,  shadowMapRT1_.imageView,  shadowMapRT2_.imageView, shadowPassDepthStencilBuffer.imageView };
    ...
    render::pipelineLayoutCreate(context, shadowDescriptorSetLayouts, 3u, nullptr, 0u, &shadowPipelineLayout_);
    ...
    shadowPipelineDesc.blendState.resize(3);
    shadowPipelineDesc.blendState[0].colorWriteMask = 0xF;
    shadowPipelineDesc.blendState[0].blendEnable = VK_FALSE;
    shadowPipelineDesc.blendState[1].colorWriteMask = 0xF;
    shadowPipelineDesc.blendState[1].blendEnable = VK_FALSE;
    shadowPipelineDesc.blendState[2].colorWriteMask = 0xF;
    shadowPipelineDesc.blendState[2].blendEnable = VK_FALSE;
  ...
  void initializeOffscreenPass(render::context_t& context, const uvec2& size)
  {
    ...
    render::descriptor_binding_t materialBindings = { render::descriptor_t::type_e::UNIFORM_BUFFER, 0, render::descriptor_t::stage_e::FRAGMENT };
    ...
    render::shaderCreateFromGLSLSource(context, render::shader_t::FRAGMENT_SHADER, gDirectionalLightPassGIFragmentShaderSource, &directionalLightGIFragmentShader_);
    lightPipelineDesc.cullMode = VK_CULL_MODE_BACK_BIT;
    lightPipelineDesc.vertexShader = directionalLightVertexShader_;
    lightPipelineDesc.fragmentShader = directionalLightGIFragmentShader_;
    render::graphicsPipelineCreate(context, renderPass_.handle, 1u, fullScreenQuad_.vertexFormat, lightPipelineLayout_, lightPipelineDesc, &directionalLightGIPipeline_);
  }
  ...
  void buildAndSubmitCommandBuffer()
  {
    ...
    if (directionalLight_ != nullptr)
    {
      if (shadowCommandBuffer_.handle == VK_NULL_HANDLE)
      {
        ...
        {
          render::commandBufferRenderPassBegin(context, &shadowFrameBuffer_, clearValues, 4u, shadowCommandBuffer_);
          //Shadow pass
          ...
          while (objectIter != object_.end())
          {
            render::descriptorSetBind(shadowCommandBuffer_, shadowPipelineLayout_, 1, &objectIter.get().descriptorSet, 1u);
            render::descriptorSetBind(shadowCommandBuffer_, shadowPipelineLayout_, 2, &material_.get(objectIter.get().material)->descriptorSet, 1u);
    ...
    render::commandBufferBegin(context, commandBuffer_);
    {
      ...
      if (directionalLight_ != nullptr)
      {
        //render::graphicsPipelineBind(commandBuffer_.handle, directionalLightPipeline_);
        if (globalIllumination_)
        {
          render::graphicsPipelineBind(commandBuffer_, directionalLightGIPipeline_);
        }
        else
        {
...
private:
  ...
  struct directional_light_t
  {
    struct uniforms_t
    {
      ...
      maths::vec4 color;             //RGB is light color, A is ambient
      maths::mat4 worldToClipSpace;  //Transforms points from world space to light clip space
      maths::vec4 shadowMapSize;
      maths::vec3 padding;
      float sampleCount = 400;
      maths::vec4 samples[400];
    }uniforms;
